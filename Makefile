...
sonarqube.scan.docker.log:
	@docker run \
		--network=host \
    -e SONAR_HOST_URL="$(SONARQUBE_URL)" \
    -e SONAR_SCANNER_OPTS="-Dsonar.projectKey=$(SONARQUBE_PROJECT_KEY) -Dsonar.login=$(SONARQUBE_TOKEN)" \
...
test.coverage.report:
	...
	@python -m pytest --cov-report xml:$(PROJECT_BASE_DIR)/coverage.xml --cov=src --junitxml=$(PROJECT_BASE_DIR)/result.xml $(PROJECT_BASE_DIR)
